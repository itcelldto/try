...
function generateCaptcha() {
    const characters = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; // Remove lowercase letters
    let captcha = '';
    const length = 6;
...
function displayCaptcha() {
    ...
    // Store ONLY uppercase version for validation
    sessionStorage.setItem('captcha', captchaText);
    $('#captcha-text').text(captchaText); // Display as uppercase only
    $('#captcha-input').val('');
    $('#captcha-error').hide();
}

function validateCaptcha() {
    const userInput = $('#captcha-input').val().trim().toUpperCase(); // Convert input to uppercase
    const storedCaptcha = sessionStorage.getItem('captcha');
    
    // Compare exactly (both in uppercase)
...
$(document).ready(function() {
    ...
    // CAPTCHA input event - auto convert to uppercase
        ...
        fetch(settingsUrl)
            ...
            .then(data => {
                ...
                if (loginSuccessful) {
                    ...
                    displayCaptcha(); // Generate new CAPTCHA on login failure
                }
            })
            .catch(error => {
                console.error('Error:', error);
                $('#login-error').text('Error connecting to server. Please try again later.').show();
                displayCaptcha(); // Generate new CAPTCHA on error
            })
            .finally(() => {
                // Re-enable button
